...
# Use 2023 NSHA catalogue
print('Declustering NSHA23CAT')

pklfile = path.join('2023_working','merged_cat_pref_mags.pkl')
nsha_dict = pickle.load(open(pklfile, 'rb'))

# set HMTK file name
if prefmag1 == 'orig':
    hmtk_csv = path.join('data','NSHA23CAT_V0.1_hmtk_mx_orig.csv')
elif prefmag1 == 'mw' and max_time_ceil == False:
    hmtk_csv = path.join('data','NSHA23CAT_V0.1_hmtk.csv')
elif prefmag1 == 'mw' and max_time_ceil == True:
    hmtk_csv = path.join('data','NSHA23CAT_V0.1_hmtk_trunc.csv')


# write HMTK csv
nsha2hmtk_csv(nsha_dict, hmtk_csv, prefmag1)
...
#########################################################################
# merge extra columns removed by HMTK parser - not sure this is needed in 2023!
#########################################################################
"""
...
for i in range(0, len(nsha_dict)):
    datestr.append(''.join((str(nsha_dict[i]['DATETIME'].year), str('%02d' % nsha_dict[i]['DATETIME'].month), \
                       str('%02d' % nsha_dict[i]['DATETIME'].day), str('%02d' % nsha_dict[i]['DATETIME'].hour), \
                       str('%02d' % nsha_dict[i]['DATETIME'].minute))))
